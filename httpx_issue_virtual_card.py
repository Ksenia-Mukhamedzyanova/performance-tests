...
issue_virtual_card_account_payload = {
    "userId" : create_user_response_data['user']['id'],
    "accountId": open_debit_card_account_response_data['account']['id']
}

issue_virtual_card_account_response = httpx.post(
    "http://localhost:8003/api/v1/cards/issue-virtual-card",
    json=issue_virtual_card_account_payload
)

issue_virtual_card_account_response_data = issue_virtual_card_account_response.json()

print("Issue virtual card response:", issue_virtual_card_account_response_data)
print("Issue virtual card status code:", issue_virtual_card_account_response.status_code)